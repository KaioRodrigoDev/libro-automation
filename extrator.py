...
class ExtratorCartaoResposta:
    ...
    def carregar_e_pre_processar(self):
        ...
        if self.imagem_original is None:
            raise FileNotFoundError(f"Imagem n o encontrada: {self.caminho_imagem}")
    ...
    def extrair_respostas_por_bloco(self, pontos_bloco):
        roi_cinza, matriz = self.recortar_bloco_inclinado(pontos_bloco)
        ...
        _, matriz_inv = cv2.invert(matriz)
        ...
        for linha in range(self.linhas):
            ...
            for coluna in range(self.colunas):
                ...
                cx = inicio_x + passo_x // 2
                cy = inicio_y + passo_y // 2
                pt_warped = np.array([[[cx, cy]]], dtype="float32")
                pt_orig = cv2.perspectiveTransform(pt_warped, matriz_inv)
                centros_linha.append((int(pt_orig[0][0][0]), int(pt_orig[0][0][1])))

            centros_bloco.append(centros_linha)
            ...
            if max_pixels > limiar_ruido:
                ...
                respostas_bloco.append("P")

        return respostas_bloco, centros_bloco
    ...
    def executar(self, blocos_coordenadas):
        ...
        for pontos_bloco in blocos_coordenadas:
            respostas, centros = self.extrair_respostas_por_bloco(pontos_bloco)
            gabarito_final.extend(respostas)
            centros_finais.extend(centros)

        return gabarito_final, centros_finais
...
if __name__ == "__main__":
    ...
    respostas, centros = extrator.executar(coords_blocos_exemplo)
    print(respostas)
